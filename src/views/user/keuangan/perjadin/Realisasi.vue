      ...
      <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
        <div class="justify-center items-center w-full md:inset-0 h-modal md:h-full">
          <div class="relative p-4 w-full h-full md:h-auto flex flex-col space-y-4">
            <div class="flex flex-row justify-between items-center pb-4 rounded-t border-b dark:border-gray-600">
              <Breadcumb />
              <span
              ...
              >
              <span v-else class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{{
            ...
            <div class="flex flex-row justify-between">
              ...
              <div class="flex items-center space-x-1 text-center justify-end" v-if="authStore.user.role == 'ADMIN'">
                <button
                  v-if="perjadinDetailStore.singleResponse.status == 'UNVERIFIED'"
                ...
                <button
                  ...
                  v-else-if="perjadinDetailStore.singleResponse.status == 'VERIFIED' && perjadinDetailStore.singleResponse.master.status !== 'SELESAI'"
                  type="button"
                  class="text-gray-900 w-fit flex flex-row space-x-4 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                >
                  Unverified
              ...
              <div
                ...
                v-if="authStore.user.role == 'USER' && perjadinDetailStore.singleResponse.status == 'UNVERIFIED'"
              >
                <button
                ...
                <button
                  ...
                  class="text-gray-900 w-fit flex flex-row space-x-4 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
                >
                  <CheckCircleIcon class="h-5" /><span>Update</span>
                </button>
              </div>
            </div>
            ...
            >
              ...
              <div><span class="font-medium">Info!</span> Verifikasi kelengkapan data pertanggung jawaban</div>
            </div>

            <div v-else class="flex items-center p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
              <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                <path
                  d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"
                />
              ...
              <span class="sr-only">Info</span>
              <div><span class="font-medium">Info!</span> Selalu periksa catatan untuk melihat catatan dari Verifikator !</div>
            </div>

            <div class="flex flex-col min-h-[75vh]">
              <div class="flex flex-row space-x-6 min-h-[75vh]">
                <ol class="space-y-2 w-1/5">
                ...
                <div class="w-4/5 flex flex-col justify-between">
                  ...
                  <KeepAlive>
                    <Hotel :key="hotelRef" v-if="currentStep == 1" :disabledForm="disabledForm" />
                  </KeepAlive>
                  <KeepAlive>
                    <UangHarian :key="uhRef" v-if="currentStep == 2" :disabledForm="disabledForm" />
                  </KeepAlive>

                  <KeepAlive>
                    <Pesawat :key="pesawatRef" v-if="currentStep == 3" :disabledForm="disabledForm" />
                  </KeepAlive>

                  <KeepAlive>
                    <Transport :key="transportRef" v-if="currentStep == 4" :disabledForm="disabledForm" />
                  </KeepAlive>

                  <KeepAlive>
                    <TaksiJakarta :key="taksiJakartaRef" v-if="currentStep == 5" :disabledForm="disabledForm" />
                  </KeepAlive>

                  <KeepAlive>
                    <TaksiTujuan :key="taksiTujuanRef" v-if="currentStep == 6" :disabledForm="disabledForm" />
                  </KeepAlive>
                  <KeepAlive>
                    <Representatif :key="repRef" v-if="currentStep == 7" :disabledForm="disabledForm" />
                  </KeepAlive>
                  <KeepAlive>
                    <Lampiran :key="lampiranRef" v-if="currentStep == 8" :disabledForm="disabledForm" />
                  </KeepAlive>
      ...
      <ConfirmationDialog :overflowVisible="true" :show="confirmDialog" @submit="submit()" @close="confirmDialog = !confirmDialog" :canSubmit="true">
        ...
        <template #content>
          <span class="text-justify w-full text-gray-700"
            >Pertanggung jawaban {{ perjadinDetailStore.singleResponse.nama }} akan di perbaharui untuk di Verifikasi Admin
          </span>
        </template>
      </ConfirmationDialog>

      <ConfirmationDialog :overflowVisible="true" :show="verifiedDialog" @submit="submitVerfied()" @close="verifiedDialog = !verifiedDialog" :canSubmit="true">
        <template #title>
          <h1>Verified ?</h1>
        </template>

        <template #content>
          <span v-if="perjadinDetailStore.updateData.status == 'VERIFIED'" class="text-justify w-full text-gray-700"
            >Apa anda yakin data pertanggung jawaban ini sudah lengkap?
          </span>
          <span v-else class="text-justify w-full text-gray-700">Apa anda yakin data pertanggung jawaban ini akan dibatalkan Verifikasinya? </span>
        </template>
      </ConfirmationDialog>
    </template>
  </div>
...
function verifiedSubmit(x) {
  if (x == true) {
    perjadinDetailStore.$patch((state) => {
      ...
      state.updateData.catatan = 'SPPD NO ' + perjadinDetailStore.singleResponse.no_sppd + ' Telah di Verifikasi'
    })
  } else {
    perjadinDetailStore.$patch((state) => {
      state.updateData.status = 'UNVERIFIED'
      state.updateData.catatan = 'SPPD NO ' + perjadinDetailStore.singleResponse.no_sppd + ' Telah di batalkan Verifikasinya'
    })
  }
