    ...
    <template v-else>
      <div class="justify-center items-center w-full md:inset-0 h-modal md:h-full">
        <div class="relative p-4 w-full h-full md:h-auto flex flex-col space-y-4">
          ...
          <!-- Modal content -->
          <span v-if="isEditAll" class="bg-green-100 text-green-800 text-sm font-medium me-2 px-2.5 py-2.5 rounded dark:bg-green-900 dark:text-green-300">
            EDIT MODE</span
          >
          <Perencanaan v-if="currentStep == 0" :isEdit="isEditAll" />
          <Detail :isEdit="isEditAll" v-if="currentStep == 1" @openModal="openModal" @deletePegawai="deletePegawai()" />
          ...
          <div class="flex flex-row justify-between">
            ...
            <div
              v-else-if="authStore.role == 'ADMIN' && perjadinStore.singleResponse.status == 'VERIFIKASI'"
              class="flex items-center space-x-1 mt-4 text-center justify-end"
            >
              <button
      ...
      <Dialog :overflowVisible="true" :show="verifikasiDialog" @submit="updateStatusData()" @close="verifikasiDialog = !verifikasiDialog" :canSubmit="true">
        <template #title>
...
import { ArrowDownTrayIcon, DocumentTextIcon, TrashIcon, CheckCircleIcon, ArrowPathIcon, PencilSquareIcon, PaperAirplaneIcon } from '@heroicons/vue/24/outline'
...
const steps = computed(() => {
  return ['Perencanaan', 'Detail', 'Lampiran', ...(perjadinStore.singleResponse.status == 'PERTANGGUNG JAWABAN' ? ['Realisasi'] : [])]
})
...
async function updateStatusData() {
  ...
  const result = await perjadinStore.updateStatus(perjadinStore.singleResponse.id)
  if (result.status) {
    toast.update(id, {
      render: 'Berhasil !!',
      position: toast.POSITION.BOTTOM_CENTER,
