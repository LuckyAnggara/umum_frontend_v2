  ...
  <div class="mx-auto w-full px-4">
    ...
    <template v-else>
      <div class="flex flex-col space-y-4 bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
              ...
              <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                  <div class="flex items-center">
                    <ChevronRightIcon class="rtl:rotate-180 w-3 h-3 text-gray-400 mx-1" />
        ...
        <div class="w-full px-6 place-self-end text-left flex flex-row space-x-4">
          <div class="flex flex-col space-y-2">
            <div>
              <label for="name" class="block font-bold text-gray-900 dark:text-white">Tahun Anggaran</label>
              <div class="relative w-full">
                <span>{{ makStore.singleResponse.tahun_anggaran }}</span>
              </div>
            </div>
            <div>
              <label for="name" class="block font-bold text-gray-900 dark:text-white">Mata Anggaran Kegiatan</label>
              <div class="relative w-full">
                <span>{{ makStore.singleResponse.kode_mak }} - {{ makStore.singleResponse.keterangan }}</span>
              </div>
            </div>

            <div class="text-left">
              <label for="name" class="block font-bold text-gray-900 dark:text-white">Total Pagu</label>
              <span>{{ IDRCurrency.format(makStore.singleResponse.anggaran) }}</span>
            </div>
            <div class="text-left">
              <label for="name" class="block font-bold text-gray-900 dark:text-white">Realisasi</label>
              <span>{{ IDRCurrency.format(makStore.totalSingleSudahRealisasi) }}</span>
            </div>
            <div class="text-left">
              <label for="name" class="block font-bold text-gray-900 dark:text-white">Sisa Pagu</label>
              <span>{{ IDRCurrency.format(makStore.singleResponse.anggaran - makStore.totalSingleSudahRealisasi) }}</span>
            </div>
          </div>
        ...
        <div class="overflow-y-visible w-full scrollbar-thin scrollbar-track-gray-500 scrollbar-thumb-gray-700">
          <table class="lg:w-full min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              ...
              <tr v-else-if="!makStore.isLoading && makStore.singleResponse.detail.length < 1">
              ...
              >
                ...
                <td class="px-4 py-1">
                  <span>{{ IDRCurrency.format(item.total_anggaran - item.total_realisasi) }}</span>
                </td>
                <td class="px-4 py-1">
                  <span
            ...
            <tfoot>
              ...
              <td class="px-4 py-1 font-semibold">
                <span>{{ IDRCurrency.format(makStore.totalSingleAnggaran) }}</span>
              </td>
              <td class="px-4 py-1 font-semibold">
                <span>{{ IDRCurrency.format(makStore.totalSingleSudahRealisasi) }}</span>
              </td>
              <td class="px-4 py-1 font-semibold">
                <span>{{ IDRCurrency.format(makStore.totalSingleAnggaran - makStore.totalSingleSudahRealisasi) }}</span>
              </td>
            </tfoot>
          </table>
        </div>
