          ...
          <div class="flex items-center justify-center flex-row">
            ...
            >
              <button
                ...
                class="text-gray-900 w-fit flex flex-row space-x-2 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
              >
                <FolderPlusIcon class="h-5" /> <span>Revisi</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <table class="lg:w-full min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" class="px-4 py-3">MAK</th>
              <th scope="col" class="px-4 py-3">Program/ Kegiatan/ KRO/ RO/ Komponen</th>
        ...
        <table class="lg:w-full min-w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-1">
                <span v-if="item.type == 'detail'">{{ IDRCurrency.format(makStore.totalPaguDigunakan(item.id)) }}</span>
              </td>
              <td class="px-4 py-1">
                <span v-if="item.type == 'detail'">{{ IDRCurrency.format(item.jumlah - makStore.totalPaguDigunakan(item.id)) }}</span>
              </td>
            </tr>
          </tbody>
          <tfoot>
            ...
            <td class="px-4 py-1 font-bold">
              <span>{{ IDRCurrency.format(makStore.totalPenggunaanPaguNominatif) }}</span>
            </td>
            <td class="px-4 py-1 font-bold">
              <span>{{ IDRCurrency.format(makStore.totalPaguNominatif - makStore.totalPenggunaanPaguNominatif) }}</span>
            </td>
          </tfoot>
        </table>
      </div>
...
async function deleteData() {
  ...
  const result = await makStore.destroy(deleteId.value)
  if (result.success) {
    toast.update(id, {
      render: 'Berhasil !!',
      position: toast.POSITION.BOTTOM_CENTER,
