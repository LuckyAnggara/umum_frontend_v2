  ...
  <div>
    <div class="max-w-md mx-auto px-6 py-8 md:max-w-lg">
      <div class="flex items-center">
        ...
        <h2 class="ml-2 font-semibold text-gray-800 text-xl">Permintaan Pelayanan BMN</h2>
      </div>
      <div class="mt-4 bg-white rounded-lg overflow-hidden border border-gray-400">
        ...
        <div class="flex flex-col divide-y divide-gray-200 p-4">
          <div class="p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400" role="alert">
            <span class="font-medium">Info!</span> scan barcode nup untuk mempermudah pencarian
          </div>

          <div>
            <label class="inline-flex items-center cursor-pointer my-2">
              <input type="checkbox" v-model="cariByNama" class="sr-only peer" />
              <div
                class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"
              ></div>
              <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">Cari dari Nama</span>
            </label>
            <template v-if="cariByNama">
              ...
              <div class="relative">
                ...
                >
                  <MagnifyingGlassIcon v-if="!bmnStore.isLoading" class="w-5 h-5" />
      ...
      <template v-else>
        <div class="mt-4 bg-white rounded-lg overflow-hidden">
          <div class="grid grid-cols-2 gap-4">
            ...
            >
              <div class="mb-4">
                <img class="w-full h-36 object-cover rounded" :src="showImage(item.image)" alt="Product Image" />
                <a>
                  <h5 class="text-md font-semibold mt-2 tracking-tight text-gray-900 dark:text-white">
                    {{ item.nama }}
                  </h5>
                </a>
              </div>
                  ...
                  <DialogPanel class="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                    ...
                    <div class="mt-4 flex space-x-4">
                      <button
                        ...
                        @click="submit"
                      >
                        <span v-if="!layananBmnStore.isStoreLoading"> Proses </span>
...
async function submit() {
  ...
  const success = await layananBmnStore.store(bmnStore.singleResponsesPermintaan.nup)
  if (success.status) {
    toast.update(id, {
      render: 'Berhasil !!',
      position: toast.POSITION.BOTTOM_CENTER,
...
async function cariPegawai() {
  ...
  try {
    const response = await fetch(`https://lapkin.bbmakmur.com/api/employee-show/${layananBmnStore.form.nip}`)
      .then((response) => {
