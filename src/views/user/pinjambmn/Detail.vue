    ...
    <div class="max-w-md mx-auto px-6 py-8 md:max-w-lg">
      ...
      <form autocomplete="off" class="mt-4 bg-white rounded-lg overflow-hidden border border-gray-400" @submit.prevent="confirm()">
        ...
        <div v-if="bmnStore.singleResponsesPermintaan" class="p-4 flex flex-col space-y-2">
          ...
          <template v-if="bmnStore.singleResponsesPermintaan.mobil_dinas == true">
            <div>
          ...
          <div v-if="!canMobilDinas" class="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
            <span class="font-medium">Unit ini sedang mempergunakan mobil dinas lainnya </span>
          </div>

          <template v-if="canMobilDinas">
            <div class="text-left">
...
const canMobilDinas = computed(() => {
  if (bmnStore.singleResponsesPermintaan.pinjam?.unit !== peminjamanBmnStore.form.unit) {
    return false
  }
  return true
})
