<template>
  <section class="dark:bg-gray-900 p-3 sm:p-5">
    <div class="mx-auto max-w-screen-2xl px-6 py-4 bg-white shadow-lg rounded-lg">
      <div class="py-4 border-gray-200 w-full flex flex-row justify-between items-center">
        <div class="flex flex-col space-y-4 w-2/3">
          <div class="text-left w-1/2">
            <label for="name" class="block font-medium text-gray-900 dark:text-white">Pimpinan</label>
            <select
            ...
            >
              <option :key="item.id" v-for="item in mainStore.pimpinanOptions" :value="item.id">
                {{ item.label }}
              </option>
            </select>
          </div>
          <div class="flex flex-row space-x-2 w-1/2">
            <div class="text-left w-full">
              <label for="name" class="block font-medium text-gray-900 dark:text-white">Bulan</label>
              <select
                @change="agendaStore.getData()"
                v-model="agendaStore.filter.month"
                class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              >
                <option :key="item.id" v-for="item in mainStore.bulanOptions" :value="item.id">
                  {{ item.label.toUpperCase() }}
                </option>
              </select>
            </div>
            <div class="text-left w-full">
              <label for="name" class="block font-medium text-gray-900 dark:text-white">Tahun</label>
              <select
                @change="agendaStore.getData()"
                v-model="agendaStore.filter.year"
                class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
              >
                <option :key="item" v-for="item in mainStore.tahunOptions" :value="item">
                  {{ item }}
                </option>
              </select>
            </div>
