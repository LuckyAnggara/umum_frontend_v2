<template>
      ...
      <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-visible">
          ...
          <div class="w-full flex space-x-3">
            <div class="flex items-center">
              ...
              >
                <option
                  :value="limit.value"
                  :selected="bmnStore.filter.currentLimit == limit.value ? true : false"
        ...
        <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
            Showing
            <span class="font-semibold text-gray-900 dark:text-white">{{ bmnStore.from }} - {{ bmnStore.to }}</span>
            of
            <span class="font-semibold text-gray-900 dark:text-white">{{ bmnStore.total }}</span>
          </span>
          <ul class="inline-flex items-stretch -space-x-px">
            ...
            <li>
              <a
                @click="bmnStore.lastPage == bmnStore.currentPage ? '' : nextPage()"
                :class="
  ...
  <NewModal :show="newModal" @close="newModal = false" @submit="newStore" @fileChange="uploadFile" />

  <DetailModal :show="editModal" @close="editModal = false" @submit="newUpdate" @fileChange="uploadFile" />
